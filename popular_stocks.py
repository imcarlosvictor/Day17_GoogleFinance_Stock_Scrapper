...
def find_stock(*position: int) -> str:
    ...
    for num in position:
        ...
        price_increase = 'M4 12l1.41 1.41L11 7.83V20h2V7.83l5.58 5.59L20 12l-8-8-8 8z'  # Arrow up symbol

        if percent_change == price_increase:
            print(
                f'{company_name.get_text()} [{stock_symbol.get_text()}] \n{user_following.get_text()}\t +{percent.get_text()}\n'
            )
        elif percent_change != price_increase:
            print(
                f'{company_name.get_text()} [{stock_symbol.get_text()}] \n{user_following.get_text()}\t -{percent.get_text()}\n'
            )
